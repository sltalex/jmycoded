...
!(async () => {
    ...
    let authorCode = []
    let res = await getAuthorShareCode('https://raw.githubusercontent.com/test/updateTeam/main/shareCodes/tiger.json')
    if (!res) {
        res = await getAuthorShareCode('https://raw.fastgit.org/test/updateTeam/main/shareCodes/tiger.json')
    }
    if (res) {
...
async function getShareCodePool(key, num) {
    ...
    for (let i = 0; i < 2; i++) {
        try {
            const { body } = await got(`https://api.jdsharecode.xyz/test/${key}/${num}`)
