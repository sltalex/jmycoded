...
!(async () => {
  ...
  $.res = await getAuthorShareCode('https://raw.test.com/Aaron-lv/updateTeam/master/shareCodes/jxmc.json')
  if (!$.res) {
    $.http.get({url: 'https://purge.jsdtestelivr.net/gh/Aaron-lv/updateTeam@master/shareCodes/jxmc.json'}).then((resp) => {}).catch((e) => console.log('刷新CDN异常', e));
    await $.wait(1000)
    $.res = await getAuthorShareCode('https://cdn.test.net/gh/Aaron-lv/updateTeam@master/shareCodes/jxmc.json')
  }
  $.res = [...($.res || []), ...(await getAuthorShareCode('https://raw.test.org/zero205/updateTeam/main/shareCodes/jxmc2.json') || [])]
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `https://test.nz.lu/jxmc`, timeout: 30 * 1000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
