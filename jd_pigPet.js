...
let shareId = 'KMnydTyM9ezAduo11QsTZZsAdoUJQ3Dik'
$.shareCodes = [];
if ($.isNode()) {
...
!(async () => {
  ...
  if (process.env.PIGPETSHARECODE) {
    ...
    let res = await getAuthorShareCode('https://raw.githubusercontent.com/test/updateTeam/main/shareCodes/pigPet.json')
    if (!res) {
      res = await getAuthorShareCode('https://raw.fastgit.org/test/updateTeam/main/shareCodes/pigPet.json')
    }
    if (res){
  ...
  console.log(`\n======开始大转盘助力======\n`);
  $.helpId = await getAuthorShareCode('https://raw.fastgit.org/test/updateTeam/main/shareCodes/pig.json');
