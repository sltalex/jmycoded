...
!(async () => {
  ...
  if (process.env.PIGPETSHARECODE) {
    ...
    let res = await getAuthorShareCode('https://raw.stest.com/shufflewzc/updateTeam/main/shareCodes/pigPet.json')
    if (!res) {
      res = await getAuthorShareCode('https://raw.test.org/shufflewzc/updateTeam/main/shareCodes/pigPet.json')
    }
    if (res){
  ...
  console.log(`\n======开始大转盘助力======\n`);
  $.helpId = await getAuthorShareCode('https://raw.test.org/shufflewzc/updateTeam/main/shareCodes/pig.json');
              ...
              if (data.resultData.resultCode === 0 && n < 5) {
                ...
                for (let i = 0; i < $.friends.length; i++) {
                  if ($.friends[i].status === 1) {
                    ...
                    if (!['shuewzc', 'x05'].includes($.name)) { //放过孩子吧TT
