...
const inviteCodes = [
  `YEdzPbDnLg@e0RzOb_qOaJx@eU9Ya-3gZfsg9WeDnyJG1g`,
]
if ($.isNode()) {
...
!(async () => {
  ...
  await requireConfig()
  $.authorCode = await getAuthorShareCode('https://raw.githubusercontent.com/test-lv/test/master/shareCodes/jd_updateCash.json')
  if (!$.authorCode) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/test-lv/updateTeam@master/shareCodes/jd_updateCash.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.authorCode = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/test-lv/updateTeam@master/shareCodes/jd_updateCash.json') || []
  }
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `http://#/api/v1/jd/jdcash/read/${randomCount}/`, 'timeout': 30000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
